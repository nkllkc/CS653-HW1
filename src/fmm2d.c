...
int main(int argc, char **argv)
{
    ...
    int j;
    double diff, max_diff = 0.0, error, g_flops;
    clock_t t1, t2, t3, t4;
...
------------------------------------------------------------------------------*/
    ...
    for (l = L - 1; l >= 0; l--)
    {
        ...
        for (c = c0[l]; c < c0[l] + nc; c++)
        {
            ...
            vc[0] = (c - c0[l]) / lc;
            vc[1] = (c - c0[l]) % lc; /* Mother's vector cell ID */
...
------------------------------------------------------------------------------*/
    ...
    for (l = 2; l <= L; l++)
    {
        ...
        for (c = c0[l]; c < c0[l] + nc; c++)
        { /* Loop over daughter cells */
            vc[0] = (c - c0[l]) / lc;
            vc[1] = (c - c0[l]) % lc; /* Daughter's vector cell ID */
            for (b = 0; b < 2; b++)
        ...
        { /* Loop over cells c*/
            vc[0] = (c - c0[l]) / lc;
            vc[1] = (c - c0[l]) % lc; /* Vector cell ID */
            for (b = 0; b < 2; b++)
            { /* Beginning & ending interactive-cell indices */
                vcb[b] = 2 * (vc[b] / 2 - 1);
...
------------------------------------------------------------------------------*/
    ...
    for (c = c0[L]; c < c0[L] + nc; c++)
        head[c] = EMPTY; /* Reset the headers, head */

    /* Scan particles to construct headers, head, & linked lists, lscl */
    for (j = 0; j < Npar; j++)
    {
        for (b = 0; b < 2; b++) {
            vc[b] = z[j][b] / rc;       /* Vector cell index */
            number_of_flops += 1;
        }
    ...
    for (vc[0] = 0; vc[0] < lc; (vc[0])++)
        for (vc[1] = 0; vc[1] < lc; (vc[1])++)
        {
            ...
            if (head[c] == EMPTY)
                continue; /* Skip this cell if empty */
            ...
            for (vc1[0] = vcb[0]; vc1[0] <= vce[0]; (vc1[0])++)
                for (vc1[1] = vcb[1]; vc1[1] <= vce[1]; (vc1[1])++)
                {
                    ...
                    if (head[c1] == EMPTY)
                        continue; /* Skip this neighbor cell if empty */
                    ...
                    while (j != EMPTY)
                    {
                        ...
                        while (k != EMPTY)
                        {
                            ...
                            if (j < k)
                            {
                                ...
                                rjk = sqrt(rjk);
                                number_of_flops += FSQRT;

                                pot[j] += q[k] * log(rjk);
                                number_of_flops += 1 + 1 + FLOG;
